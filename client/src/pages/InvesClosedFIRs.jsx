...
export const InvesClosedFIRs = () => {
  ...
  const closedFIRs = firs.filter((elem)=> elem.status === "closed");
  console.log(firs);
  ...
  Hey Admin, Below are the ALL FIRs, click on a link to see a detailed view. Pending FIRs are those to whom the investigator has not been assigned yet. All FIRs contains all type of FIRs, like Pending, Active, Completed, Closed. You can filter them from the below given filter, explore them.
</h1>
        ...
        >
          Closed FIRs ({closedFIRs.length})
        </h1>
      {closedFIRs&& closedFIRs.length > 0 ? (
        <div losede="relosedverflow-x-auto shadow-md sm:rounded-lg mx-2 md:mx-5 lg:mx-10 xl:mx-20 mb-20">
  <div className="flex flex-col sm:flex-row items-center justify-between pb-4">
    ...
    <table className="w-full text-sm text-left text-gray-600 "> 
        ...
        <tbody>
  {closedFIRs &&  closedFIRs.map((fir) => (
    <tr clalosedhite bolosedkey={fir._id}>
