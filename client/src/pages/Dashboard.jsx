...
<div>
  ...
  <h1 className='ml-4 md:ml-16 text-sm md:text-md lg:text-lg font-bold font-custom text-center'>
    Welcome Back {user?.name}, you can view all your FIRs on this Dashboard and can chat with the investigator of your respective case.
  </h1>
  <br />
  ...
  <div className='flex flex-col md:flex-row'>
    {data.map((item) => (
      <div key={item.id} className=' mx-4 my-2 md:mx-0 md:my-4'>
        <a href="#" className=" ml-16 block max-w-xs p-4 bg-white border border-gray-200 rounded-lg shadow bg-gradient-to-t from-gray-400 via-gray-500 to-gray-700 transform scale-100 hover:scale-105 transition-transform duration-300 ease-in-out">
          <h5 className="mb-1 md:mb-2 text-lg md:text-xl font-bold tracking-tight text-gray-900 dark:text-white">{item.title}</h5>
