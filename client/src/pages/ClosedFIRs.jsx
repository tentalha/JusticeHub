...
import { getClosedFIRS } from "features";

export const ClosedFIRs = () => {
  ...
  return (
    <div className="xl:flex-1 xl:overflow-y-auto xl:ml-52 xs:ml-0 lg:flex-1 lg:overflow-y-auto lg:ml-52 md:flex-1 md:overflow-y-auto md:ml-52 sm:flex-1 sm:overflow-y-auto sm:ml-52">
      ...
      <br />
      <h1 id="tagline" className=" mt-2 bg-gray-100 hover:text-white hover:bg-black py-1 px-2 rounded-2xl  sm:ml-0 sm:mr-0 text-sm sm:text-sm md:text-sm lg:text-sm font-bold font-custom text-center transform scale-100 hover:scale-95 transition-transform duration-300 ease-in-out">
  Hey Admin, Below are the Closed FIRs, click on a link to see a detailed view. Closed FIRs are those to whom the investigator was assigned but due to very weak evidences and no such progress from the complainant, that case has been closed.
</h1>

        <h1
          id="tagline"
          className="p-5 mt-2 text-center  sm:ml-0 text-xl sm:text-2xl lg:text-4xl font-bold font-custom bg-clip-text text-transparent bg-gray-500"
        >
          Closed FIRs ({firs.length})
        </h1>
      {firs&& firs.length > 0 ? (
