...
export const AssignedFIRs = () => {
  const [isDropdownOpen, setDropdownOpen] = useState(false)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [assignedFIRs, setAssignedFIRs] = useState([])
  ...
  useEffect(() => {
    const assignedFirs = firs.filter((elem) => elem?.status === 'active')
    setAssignedFIRs(assignedFirs)
  }, [firs])
  ...
  return (
    <div className="xl:flex-1 xl:overflow-y-auto xl:ml-52 xs:ml-0 lg:flex-1 lg:overflow-y-auto lg:ml-52 md:flex-1 md:overflow-y-auto md:ml-52 sm:flex-1 sm:overflow-y-auto sm:ml-52">
      <div className={`${isModalOpen ? 'blur-sm' : ''}`}>
        ...
        >
          Hey Admin, Below are the ALL FIRs, click on a link to see a detailed
          view. Pending FIRs are those to whom the investigator has not been
          assigned yet. All FIRs contains all type of FIRs, like Pending,
          Active, Completed, Closed. You can filter them from the below given
          filter, explore them.
        </h1>
        ...
        >
          Assigned FIRs ({assignedFIRs.length})
        </h1>
        {assignedFIRs.length && assignedFIRs.length > 0 ? (
